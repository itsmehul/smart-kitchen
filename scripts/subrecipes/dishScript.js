...
transformedDishes = Object.values(x).map(dish=>{
  ...
  for(let step of dish.steps){
    if(!step?.utensilName || step.utensilName===" "){
      continue
    }
    
    steps.push({stepNo: count++,
    qty: step?.quantity,
    action: {id: mappedUtensils[step?.utensilName]},
  ...
  type: dish.type && dish.type[0].toUpperCase() + dish.type.substring(1),
  station: {name:dish.station},
  category:{
    id: dish.categoryId,
...
fs.writeFile('./subrecipes/parsedRecipes.js', JSON.stringify(transformedDishes), () => {
  console.log('fone');
});
